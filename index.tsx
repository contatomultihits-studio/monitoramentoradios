...
const GENRE_COLORS: Record<string, string> = {
  ...
  'MPB': '#0F9D8A', 'Funk': '#F2711C', 'Pagode': '#E5A800',
  'Rap/Hip Hop': '#6B2FD6', 'Eletrônica': '#0AA5C7', 'Gospel': '#7FB800',
  'Samba': '#D63A3A', 'Forró': '#A0612B', 'Reggae': '#2F9E44',
  'Jazz': '#475569', 'Jazz/Blues': '#475569', 'R&B/Soul': '#B8327A',
  'Alternative': '#8B5CF6', 'Films/Games': '#E0457B', 'Axé': '#F2B705',
  'Country': '#8A6A2F', 'Dance': '#14B8C4',
  'Nacional': '#15803D', 'Latin Music': '#C2410C',
  'Desconhecido': '#CBD5E1', 'Outros': '#94A3B8'
};
...
const getGenreColor = (genre: string): string => {
  ...
  return matchingGenre ? GENRE_COLORS[matchingGenre] : '#94A3B8';
};

// Texto escuro sobre cores claras e branco sobre cores escuras (legibilidade das etiquetas).
const getGenreTextColor = (hex: string): string => {
  const n = parseInt(hex.replace('#', ''), 16);
  const r = (n >> 16) & 255, g = (n >> 8) & 255, b = n & 255;
  return (0.299 * r + 0.587 * g + 0.114 * b) > 160 ? '#0D0056' : '#FFFFFF';
};
...
const RADIO_STREAM_URLS: Record<string, string> = {
  ...
  'ALPHA FM SÃO PAULO': 'https://ice.fabricahost.com.br/alphafmsp',
...
const YTButton = ({ artista, musica, size = 'sm' }: { artista: string; musica: string; size?: 'sm' | 'md' | 'lg' }) => {
  const cls = { sm: 'px-2.5 py-1.5 text-xs gap-1', md: 'px-4 py-2 text-xs gap-1.5', lg: 'px-5 py-3 text-sm gap-2' };
  return (
...
const RadioStreamPlayer = ({ radio, streamUrl }: { radio: string; streamUrl: string }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#0D0056] via-[#17116d] to-[#5279FF] p-6 shadow-2xl shadow-[#0D0056]/25">
      <div className="relative z-10 flex h-full flex-col justify-between gap-6">
        <div>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em] text-[#D0FF03]">
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#D0FF03]">Ouça agora</p>
            ...
            porDia.map(([dia, horarios]) => (
              <div key={dia} className="bg-slate-50 rounded-2xl p-4">
                <p className="font-black text-slate-700 text-sm mb-2 flex items-center gap-2">
                  ...
                  {formatDateBR(dia)}
                  <span className="ml-auto px-2 py-0.5 bg-[#5279FF]/15 text-[#0D0056] rounded-full text-xs font-black">
                    {horarios.length}x
                  </span>
                </p>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center gap-4 p-6 bg-gradient-to-r from-[#5279FF] to-[#0D0056] rounded-t-3xl">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#dbe3ff] text-xs font-bold mt-0.5 uppercase">{periodLabel}</p>
          </div>
            ...
            <div key={t.musica} className="flex items-center gap-3 bg-slate-50 hover:bg-[#5279FF]/10 rounded-2xl p-3 transition-all group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                  {t.genero && t.genero !== 'Desconhecido' && <span className="px-2 py-0.5 rounded-full text-[11px] font-black uppercase" style={{ backgroundColor: getGenreColor(t.genero), color: getGenreTextColor(getGenreColor(t.genero)) }}>{t.genero}</span>}
                  {t.camelot && <span className="rounded-full bg-[#D0FF03] px-2 py-0.5 text-[11px] font-black uppercase text-[#0D0056]">CAMELOT {t.camelot}</span>}
                  {t.tom_musical && <span className="text-xs font-bold uppercase text-slate-500">TOM {t.tom_musical}</span>}
                  <span className="text-xs font-bold text-slate-400">{horarios.slice(0, 3).join(' • ')}{horarios.length > 3 ? ` +${horarios.length - 3}` : ''}</span>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                {count > 1 && <span className="px-2.5 py-1 bg-[#D0FF03]/25 text-[#0D0056] rounded-full text-xs font-black">{count}x</span>}
...
const NowPlayingCard = ({ track }: { track: any }) => (
  <div className="relative overflow-hidden bg-gradient-to-br from-[#0D0056] via-[#20137f] to-[#5279FF] p-6 sm:p-8 rounded-3xl shadow-lg shadow-[#0D0056]/15 border border-white/10">
    <div className="relative z-10">
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-[#D0FF03] font-black text-sm uppercase tracking-[0.14em] flex items-center gap-2"><Sparkles size={16} /> ÚLTIMA EXECUÇÃO</span>
      </div>
      <div className="flex flex-col sm:flex-row sm:items-center gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {track.genero && track.genero !== 'Desconhecido' && <span className="px-4 py-2 rounded-full text-sm font-black uppercase shadow-lg" style={{ backgroundColor: getGenreColor(track.genero), color: getGenreTextColor(getGenreColor(track.genero)) }}>{track.genero}</span>}
            {track.bpm && <div className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/15 rounded-full"><Activity size={16} className="text-[#D0FF03]" /><span className="font-black text-white text-sm">{track.bpm} BPM</span></div>}
            {track.camelot && <div className="rounded-full bg-[#D0FF03] px-4 py-2 text-sm font-black uppercase text-[#0D0056]">CAMELOT {track.camelot}</div>}
            ...
            {track.ano_lancamento && (
              <div
                className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/15 rounded-full relative group/ano cursor-default"
                title="ANO DO LANÇAMENTO"
              >
                <CalendarDays size={16} className="text-[#D0FF03]" />
                <span className="font-black text-white text-sm">{track.ano_lancamento}</span>
                <span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-slate-900 text-white text-xs font-black uppercase px-2 py-1 rounded-lg shadow-lg opacity-0 group-hover/ano:opacity-100 transition-opacity duration-150 z-10">
                  ANO DO LANÇAMENTO
                </span>
              </div>
            )}
...
const MusicCard = ({
  ...
  <div className={`bg-white border rounded-2xl p-4 hover:shadow-md hover:border-[#5279FF]/40 transition-all duration-200 ${
    repeatCount && repeatCount >= REPEAT_THRESHOLD ? 'border-[#B8E000] ring-1 ring-[#D0FF03]' : 'border-slate-200'
  }`}>
    <div className="flex items-center gap-4">
      ...
      >
        ...
        {weeklyExecs.length > 0 && (
          <div className="absolute -bottom-1 -right-1 min-w-[18px] h-[18px] px-1 bg-[#5279FF] text-white rounded-full text-[11px] font-black flex items-center justify-center shadow-md">
            {weeklyExecs.length}
          </div>
        )}
      </button>
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-extrabold text-slate-900 text-base truncate leading-tight mb-1">{track.musica}</h3>
        <p className="font-semibold text-slate-500 text-sm truncate mb-2">{track.artista}</p>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 rounded-full">
            <Clock size={10} className="text-slate-500" />
            <span className="font-bold text-xs text-slate-600">{track.hora}</span>
          </div>
          {track.genero && track.genero !== 'Desconhecido' && (
            <span className="px-2.5 py-1 rounded-full text-xs font-black uppercase" style={{ backgroundColor: getGenreColor(track.genero), color: getGenreTextColor(getGenreColor(track.genero)) }}>{track.genero}</span>
          )}
          {track.bpm && (
            <div className="flex items-center gap-1 px-2.5 py-1 bg-slate-100 rounded-full">
              <Activity size={11} className="text-slate-500" />
              <span className="font-semibold text-xs text-slate-600">{track.bpm} BPM</span>
            </div>
          )}
          {track.camelot && (
          ...
          {track.ano_lancamento && (
            <div
              className="relative group/ano flex items-center gap-1 px-2.5 py-1 bg-slate-100 rounded-full cursor-default"
            >
              <CalendarDays size={11} className="text-slate-500" />
              <span className="font-semibold text-xs text-slate-600">{track.ano_lancamento}</span>
              <span className="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap bg-slate-900 text-white text-[11px] font-black uppercase px-2 py-0.5 rounded-md shadow-lg opacity-0 group-hover/ano:opacity-100 transition-opacity duration-150 z-10">
                ANO DO LANÇAMENTO
              </span>
            </div>
          )}
          {repeatCount && repeatCount >= REPEAT_THRESHOLD && (
            <div className="flex items-center gap-1 px-2.5 py-1 bg-[#D0FF03] rounded-full">
              <span className="font-black text-xs text-[#0D0056]">🔁 {repeatCount}x hoje</span>
            </div>
          )}
          {weeklyExecs.length > 0 && (
            <div className="flex items-center gap-1 px-2.5 py-1 bg-[#5279FF]/10 border border-[#5279FF]/25 rounded-full">
              ...
              <span className="font-black text-xs text-[#0D0056]">{weeklyExecs.length}x semana</span>
            </div>
          )}
        </div>
      </div>
...
const MetricCard = ({ icon: Icon, label, value, detail, accent = 'cyan', onClick }: { icon: any; label: string; value: string | number; detail: string; accent?: 'cyan' | 'fuchsia' | 'emerald' | 'amber' | 'violet' | 'blue'; onClick?: () => void }) => {
  const styles: Record<string, { shell: string; icon: string; text: string; glow: string }> = {
    cyan: { shell: 'bg-white border-slate-200', icon: 'bg-[#0D0056] text-[#D0FF03]', text: 'text-slate-500', glow: '' },
    fuchsia: { shell: 'bg-white border-slate-200', icon: 'bg-[#EA7F9F] text-white', text: 'text-slate-500', glow: '' },
    emerald: { shell: 'bg-white border-slate-200', icon: 'bg-[#5279FF] text-white', text: 'text-slate-500', glow: '' },
    amber: { shell: 'bg-white border-slate-200', icon: 'bg-[#5279FF] text-white', text: 'text-slate-500', glow: '' },
    violet: { shell: 'bg-white border-slate-200', icon: 'bg-[#0D0056] text-[#D0FF03]', text: 'text-slate-500', glow: '' },
    blue: { shell: 'bg-white border-slate-200', icon: 'bg-[#5279FF] text-white', text: 'text-slate-500', glow: '' },
  };
  ...
  return (
    <Component
      ...
      onClick={onClick}
      className={`relative w-full overflow-hidden rounded-2xl border ${current.shell} p-4 text-left ${onClick ? 'cursor-pointer transition-all hover:border-[#5279FF]/50 hover:shadow-md active:scale-[0.98]' : ''}`}
    >
      <div className="relative flex items-start justify-between gap-3">
        <div>
          <p className={`text-xs font-black uppercase tracking-[0.14em] ${current.text}`}>{label}</p>
          <p className="mt-2 text-2xl font-black leading-none text-slate-900">{value}</p>
          <p className="mt-2 text-xs font-medium leading-snug text-slate-500">{detail}</p>
        </div>
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${current.icon}`}>
          <Icon size={20} />
        </div>
      </div>
    </Component>
...
const MusicMetricsPanel = ({ metrics, onOpenUnique, onOpenRepeated }: { metrics: {
  ...
  <section className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
    <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.14em] text-[#EA7F9F]">Resumo musical</p>
        <h2 className="text-xl font-extrabold tracking-tight text-slate-900">Raio-X da programação</h2>
      </div>
      <p className="max-w-xl text-sm font-medium leading-relaxed text-slate-500">Métricas calculadas com os filtros atuais, sem alterar consultas, banco ou funcionamento existente.</p>
    </div>
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-[#D0FF03]/50 bg-gradient-to-br from-[#0D0056] via-[#20137f] to-[#5279FF] p-4 text-white shadow-lg shadow-[#5279FF]/20 sm:col-span-2">
        <div className="relative flex h-full items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#D0FF03]">Artista mais executado</p>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-[#EA7F9F]/60 bg-gradient-to-br from-[#EA7F9F] via-[#EA7F9F] to-[#D0FF03] p-4 text-white shadow-lg shadow-pink-200/70 sm:col-span-2">
        <div className="relative flex h-full items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#0D0056]">Música mais executada</p>
...
const DailyHighlightsCarousel = ({ highlights, metrics, onOpenUnique, onOpenRepeated }: {
  ...
  const accentStyles = {
    pink: 'from-[#0D0056] to-[#2A2290] text-white',
    lime: 'from-[#0D0056] to-[#2A2290] text-white',
    violet: 'from-[#0D0056] to-[#2A2290] text-white',
    blue: 'from-[#0D0056] to-[#2A2290] text-white',
  } as const;
  ...
  return (
    <section className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex flex-col items-center gap-2 text-center">
        ...
        <p className="max-w-xl text-sm font-medium leading-relaxed text-slate-500">Insights calculados com a data e os filtros ativos.</p>
      </div>
              ...
              <article key={highlight.eyebrow} className={`min-w-full bg-gradient-to-br px-14 py-7 sm:px-20 ${accentStyles[highlight.accent]}`} aria-hidden={index !== activeSlide}>
                <div className="flex min-h-56 flex-col items-center justify-center text-center">
                  ...
                  <div className="max-w-2xl">
                    <p className="text-xs font-black uppercase tracking-[0.14em] text-white/70">{highlight.eyebrow}</p>
                    ...
                    <p className="mt-3 text-sm font-medium leading-relaxed text-white/75">{highlight.description}</p>
                  </div>
                </div>
              </article>
            );
...
const ProgrammingVarietyPanel = ({ metrics }: { metrics: {
  ...
  return (
    <section className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#5279FF]">Inteligência da programação</p>
          <h2 className="text-xl font-extrabold tracking-tight text-slate-900">Distribuição de repetição</h2>
        </div>
        <p className="max-w-xl text-sm font-medium leading-relaxed text-slate-500">100% das execuções do dia ou filtro atual, distribuídas pela quantidade de vezes que cada música tocou.</p>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.buckets.map((bucket, index) => (
          <div key={bucket.label} className={`rounded-2xl border p-4 ${styles[index].shell}`}>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#0D0056]">{bucket.label}</p>
...
const UniqueTracksModal = ({ tracks, onClose }: { tracks: { artista: string; musica: string; count: number }[]; onClose: () => void }) => (
  <div className="fixed inset-0 z-[140] flex items-center justify-center bg-[#0D0056]/75 p-4 backdrop-blur-sm" onClick={onClose}>
    <div className="max-h-[86vh] w-full max-w-2xl overflow-hidden rounded-3xl border border-white/15 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-start justify-between gap-4 bg-gradient-to-r from-[#EA7F9F] to-[#5279FF] p-6 text-white">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#D0FF03]">Músicas únicas</p>
  ...
  <div className="fixed inset-0 z-[140] flex items-center justify-center bg-[#0D0056]/75 p-4 backdrop-blur-sm" onClick={onClose}>
    <div className="max-h-[86vh] w-full max-w-2xl overflow-hidden rounded-3xl border border-white/15 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-start justify-between gap-4 bg-[#0D0056] p-6 text-white">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#D0FF03]">Repetidas</p>
...
const TopArtistsCard = ({ radio }: { radio: string }) => {
  ...
  return (
    <>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm mb-8 border border-slate-200">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div className="flex items-center gap-4">
            <div className="bg-[#0D0056] p-3.5 rounded-2xl">
              <Trophy className="text-[#D0FF03]" size={24} />
            </div>
            <div>
              <h2 className="font-extrabold text-2xl tracking-tight text-slate-900">Artistas mais executados</h2>
              <p className="text-sm font-medium text-slate-500">
                {period === 'today' ? 'Atualizado hoje • desde a meia-noite' : `Últimos ${periodLabel}`} • toque na foto do artista para ver as músicas
              </p>
            </div>
          </div>
          {loadingPeriod && (
          ...
          {TOP_PERIOD_OPTIONS.map(opt => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-2xl font-black text-xs uppercase tracking-wide transition-all ${
                period === opt.value
                  ? 'bg-[#0D0056] text-[#D0FF03] shadow-sm'
                  : 'bg-white text-[#0D0056] border border-slate-200 hover:border-[#5279FF]'
              }`}
            >
              <span>{opt.emoji}</span>
              {opt.label}
              ...
              {visible.map((artist, idx) => {
                ...
                return (
                  <div key={artist.artista} className="flex flex-col items-center text-center group">
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all flex items-center justify-center">
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center gap-1">
                            ...
                            <span className="text-white text-[11px] font-black drop-shadow-lg">PLAYLIST</span>
                          </div>
                        </div>
                      </button>
                      {photosLoading && !photo && (
                    ...
                    <p className="font-bold text-[#0D0056] text-xs mt-1 flex items-center gap-1 justify-center">
                      <TrendingUp size={9} />
                      {artist.count} exec{artist.count !== 1 ? 'uções' : 'ução'}
                    </p>
                    {artist.genero && artist.genero !== 'Desconhecido' && (
                      <span className="mt-1 px-2 py-0.5 rounded-full text-[11px] font-black uppercase" style={{ backgroundColor: getGenreColor(artist.genero), color: getGenreTextColor(getGenreColor(artist.genero)) }}>
                        {artist.genero}
                      </span>
                    )}
                  </div>
...
const TopTracksCard = ({ radio }: { radio: string }) => {
  ...
  return (
    <>
      ...
      <section className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex items-center gap-4">
            <div className="rounded-2xl bg-[#0D0056] p-3.5">
              <Music className="text-[#D0FF03]" size={24} />
            </div>
            <div>
              <h2 className="font-extrabold text-2xl tracking-tight text-slate-900">Músicas mais executadas</h2>
              <p className="text-sm font-medium text-slate-500">
                {period === 'today' ? 'Atualizado hoje • desde a meia-noite' : `Últimos ${periodLabel}`} • toque na música para ver as execuções
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 self-start rounded-2xl border border-slate-200 bg-slate-50 p-1.5">
            <span className="px-2 text-xs font-black uppercase tracking-wider text-[#0D0056]">Exibir</span>
          ...
          {TOP_PERIOD_OPTIONS.map(opt => (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-2xl px-4 py-2 text-xs font-black uppercase tracking-wide transition-all ${
                period === opt.value
                  ? 'bg-[#0D0056] text-[#D0FF03] shadow-sm'
                  : 'border border-slate-200 bg-white text-[#0D0056] hover:border-[#5279FF]'
              }`}
            >
              <span>{opt.emoji}</span>
              {opt.label}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    {track.genero && track.genero !== 'Desconhecido' && <span className="rounded-full px-2 py-0.5 text-[11px] font-black uppercase" style={{ backgroundColor: getGenreColor(track.genero), color: getGenreTextColor(getGenreColor(track.genero)) }}>{track.genero}</span>}
                    {track.camelot && <span className="rounded-full bg-[#D0FF03] px-2 py-0.5 text-[11px] font-black uppercase text-[#0D0056]">{track.camelot}</span>}
                    {track.tom_musical && <span className="text-[11px] font-black uppercase text-slate-500">{track.tom_musical}</span>}
                    <span className="text-xs font-bold uppercase text-slate-400 group-hover:text-[#5279FF]">Ver execuções</span>
                  </div>
                </div>
...
const GenreChart = ({ data, chartRef, onSelectGenre }: {
  ...
  return (
    <div ref={chartRef} className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm mb-8 border border-slate-200">
      <div className="flex items-center gap-4 mb-6">
        <div className="bg-[#0D0056] p-3.5 rounded-2xl"><TrendingUp className="text-[#D0FF03]" size={24} /></div>
        <div>
          <h2 className="font-extrabold text-2xl tracking-tight text-slate-900">Ranking de gêneros</h2>
          <p className="text-sm font-medium text-slate-500">Comparação do filtro atual • sem gêneros desconhecidos</p>
        </div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 mb-6">
        {[
          ...
          <div key={label} className="rounded-2xl border border-[#5279FF]/15 bg-white/90 p-4 shadow-sm">
            ...
            <p className="mt-1 text-[11px] font-black uppercase tracking-wide text-[#5279FF]">{helper}</p>
          </div>
        ))}
      </div>
            ...
            <>
              <div className="mb-2 flex items-end justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-bold uppercase text-slate-500">{genre.value} execuções</p>
                </div>
              ...
              {genre.subGenres && <p className="mt-2 text-[11px] font-bold uppercase text-slate-400">Inclui: {genre.subGenres.join(', ')}</p>}
            </>
          );
          return isSelectable ? (
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={handleToggle}
        className="w-full flex items-center gap-3 pl-4 pr-4 py-3 bg-slate-50 rounded-2xl font-bold text-slate-700 border-2 border-transparent hover:border-[#5279FF]/50 focus:border-[#5279FF] focus:outline-none transition-all cursor-pointer">
        {loadingDates
          ...
          {loadingDates ? (
            ...
            displayDates.map((d, i) => (
              <button key={d} type="button" onClick={() => handleSelect(d)}
                ...
                📅 {formatDateBR(d)}
                {i === 0 && !datesLoaded && <span className="ml-2 text-xs font-black text-[#0D0056] uppercase">Hoje</span>}
                {datesLoaded && d === displayDates[0] && <span className="ml-2 text-xs font-black text-[#0D0056] uppercase">Mais recente</span>}
              </button>
            ))
          )}
        </div>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA]">
      ...
      <header className="bg-[#0D0056] border-b border-white/10 sticky top-0 z-50 shadow-md">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-3">
          ...
          <a href="/" className="flex min-w-0 items-center gap-3 sm:gap-4 group" title="Voltar para a home">
            <div className="shrink-0 bg-[#D0FF03] p-2 sm:p-3 rounded-xl sm:rounded-2xl group-hover:scale-105 transition-all">
              <Radio size={32} className="h-6 w-6 sm:h-8 sm:w-8 text-[#0D0056]" />
            </div>
            <div>
              <h1 className="whitespace-nowrap font-black text-lg sm:text-2xl tracking-tight text-white uppercase leading-none group-hover:text-[#D0FF03] transition-colors">IA NO RÁDIO</h1>
              <p className="whitespace-nowrap text-[11px] sm:text-xs font-bold text-[#D0FF03] uppercase tracking-[0.14em] mt-1">Monitoramento Musical</p>
            </div>
          </a>
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            <button onClick={exportPDF}
              className="flex items-center gap-2 rounded-xl border border-[#D0FF03]/30 bg-[#D0FF03] px-3 sm:px-4 py-2.5 text-xs font-black uppercase tracking-wider text-[#0D0056] transition-all hover:bg-[#dcff39] active:scale-95" title="Baixar PDF">
              <Download size={15} />
              <span className="hidden sm:inline">Baixar PDF</span>
            </button>
            <button onClick={() => doFetch(filters.radio, filters.date)} className="p-2.5 sm:p-4 bg-white/10 border border-white/15 rounded-2xl hover:bg-white/20 transition-all hover:scale-105 active:scale-95">
          ...
          <div className="flex gap-2 flex-wrap">
            {['Metropolitana FM', 'ALPHA FM SÃO PAULO', 'Antena 1', 'Forbes Radio', 'MIX Rio FM', 'Dumont FM', 'Gazeta FM', 'Kiss FM', 'EDUCADORA FM'].map(r => (
              <button key={r} onClick={() => handleRadioChange(r)}
                ...
                <span className={`h-2 w-2 rounded-full ${filters.radio === r ? 'bg-[#D0FF03]' : 'bg-slate-300'}`} />
                {r}
              </button>
            ))}
          </div>
      ...
      <section className="relative z-[60] overflow-visible border-b border-slate-200 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex flex-col gap-3">
            {hasActiveFilters && (
              <div className="flex justify-end">
                <button onClick={() => setFilters(f => ({ ...f, search: '', genero: '', hour: 'all', shift: 'all', bpm: 'all', ano: '', camelot: '' }))}
                  className="flex items-center justify-center gap-1.5 rounded-xl bg-[#0D0056] px-3 py-2 text-xs font-black uppercase tracking-wider text-white transition-all hover:bg-[#20137f]">
            ...
            <div className="grid grid-cols-2 gap-2 lg:grid-cols-6">
              ...
              <div className="relative">
                <select value={filters.hour} onChange={e => { setFilters(f => ({ ...f, hour: e.target.value, shift: 'all' })); setVisibleCount(9); }}
                  className="w-full appearance-none rounded-xl border border-transparent bg-slate-50 py-2.5 pl-3 pr-8 text-xs sm:text-sm font-bold text-slate-700 transition-all hover:border-[#5279FF]/50 focus:border-[#5279FF] focus:outline-none cursor-pointer">
            ...
            <div className="flex flex-col gap-2 border-t border-slate-100 pt-2 lg:flex-row lg:items-center">
              <div className="flex min-w-fit items-center gap-2">
                <p className="text-[11px] font-black uppercase tracking-[0.14em] text-[#5279FF]">Turnos</p>
                {filters.shift !== 'all' && (
                  <button onClick={() => { setFilters(f => ({ ...f, shift: 'all' })); setVisibleCount(9); }} className="text-[11px] font-black uppercase tracking-wider text-slate-400 hover:text-slate-700">
                    Limpar
                  </button>
                )}
              </div>
                ...
                {SHIFT_FILTER_OPTIONS.map(option => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-xl px-2 py-2 text-xs font-black uppercase tracking-wider transition-all active:scale-95 ${
                        active
            ...
            {hasActiveFilters && (
              <div className="flex flex-wrap gap-1.5">
                {filters.search && <span className="rounded-full border border-[#EA7F9F]/30 bg-[#EA7F9F]/15 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Busca: {filters.search}</span>}
                {filters.genero && <span className="rounded-full border border-[#5279FF]/25 bg-[#5279FF]/10 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Gênero: {filters.genero}</span>}
                {filters.hour !== 'all' && <span className="rounded-full border border-[#5279FF]/25 bg-[#5279FF]/10 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Hora: {filters.hour}:00</span>}
                {filters.shift !== 'all' && <span className="rounded-full border border-[#D0FF03]/35 bg-[#D0FF03]/15 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Locutor: {getShiftLabel(filters.shift)}</span>}
                {filters.bpm !== 'all' && <span className="rounded-full border border-[#EA7F9F]/30 bg-[#EA7F9F]/15 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">BPM: {filters.bpm === 'slow' ? 'Lento' : filters.bpm === 'moderate' ? 'Moderado' : 'Rápido'}</span>}
                {filters.camelot && <span className="rounded-full border border-[#D0FF03]/50 bg-[#D0FF03]/20 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Camelot: {filters.camelot}</span>}
                {filters.ano && <span className="rounded-full border border-[#0D0056]/20 bg-[#0D0056]/10 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-[#0D0056]">Ano: {filters.ano}</span>}
              </div>
            )}
          </div>
        </div>
            ...
            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar música na playlist..." value={filters.search}
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-4 text-sm font-semibold text-slate-800 placeholder:text-slate-400 transition-all focus:border-[#5279FF] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#5279FF]/15" />
              </div>
            </div>

            {filteredData.length > 0 && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="bg-[#0D0056] p-3 rounded-2xl"><Music size={20} className="text-[#D0FF03]" /></div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.14em] text-[#EA7F9F]">Playlist monitorada</p>
